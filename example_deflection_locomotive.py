...
## PASSENGER TRAIN RUNNING ACROSS RAILWAY BRIDGE
...
print(lowestFreq)
dt = 0.01/lowestFreq
time = np.arange(0, (span+wbase+300.0)/velocity+dt, dt)
...
with open('example_train.pkl', 'w') as f:  # Python 3: open(..., 'wb')
...
ax.set_ylim([30.0, -30.])
ax.set_yticks(np.arange(30.0, -30.0, -10.0))
ax.set_xlim([wbase, wbase+span])
ax.set_xticks(np.arange(wbase, wbase+span+10, 10.0))

beam, = ax.plot(x, w[0]*1000.0, color='dodgerblue', label='railway bridge')

# initializing mass
Q1Loc, = ax.plot(0.0, 0.02, 'o', color='slategrey', label='location Q1')
Q2Loc, = ax.plot(wbase, 0.02, 'o', color='darkslategrey', label='location Q2')
...
def animationFrames(i):
	...
	# Q1Loc.set_ydata(u[i]*50.0-massOffset*50.0)

	Q2Loc.set_xdata(velocity*time[i]+wbase)
	# Q2Loc.set_ydata(u[i]*50.0-massOffset*50.0)
